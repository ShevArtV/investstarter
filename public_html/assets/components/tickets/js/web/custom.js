var Tickets = {
    initialize: function () {
        ...
        $(document).on('submit', '#ticketForm', function (e) {
            ...
            Tickets.ticket.renewfiles();
            Tickets.ticket.save(this, $(this).find('.js-submit-btn[type="submit"]')[0]);
            e.preventDefault();
            return false;
        });
    ...
    ticket: {
        renewfiles: function(){
            $('.ticket-file:not(.deleted)').each(function(){
                $(this).find('input').val($(this).data('path'));
            });
        },
            ...
            $(form).ajaxSubmit({
                ...
                success: function (response) {
                    ...
                    var element = $('#ticket-preview-placeholder'),
                        docFlag = false,
                        docInputs = $('input[name^="doc_"]');
                    if (response.success) {
                        element.html(response.data.preview).show();
                        
                        if(docInputs.length){
                            $(docInputs).each(function(){
                                var name = $(this).attr('name');
                                if($(this).val()){
                                    $('#'+name+'_link').css('display', 'block');
                                    docFlag = true;
                                }
                            });
                            if(docFlag){
                                $('#doc-wrap').css('display', 'block');
                            }
                        }
                        
                        $('#ticket-preview-placeholder a').click(function(e){ e.preventDefault() });
                        
                        $('#content').html(editor.getData());
                        document.querySelector('#scroll-here').scrollIntoView({behavior:'smooth', block:'start'});
                        prettyPrint();
                    }
                    else {
                        element.html('').hide();
        ...
        save: function (form, button) {
            ...
            if($('#ticket-files-select').length){
                var curentCount = $('.ticket-file:not(.deleted)').length;
                    if($(form).find('[name="uploaded_files"]').length){
                        $(form).find('[name="uploaded_files"]').val(curentCount);
                    }
            }
